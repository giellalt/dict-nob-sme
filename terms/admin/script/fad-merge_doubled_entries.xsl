	...
	<xsl:for-each-group select="$mg_set/mgs/mg" group-by="./tg/t">
	  ...
	  <xsl:if test="not(count(current-group()) = 1)">
	    <mg>
	      <tg xml:lang="sme">
		<t>
		  <xsl:copy-of select="current-group()[1]/tg/t/@pos"/>
		  <xsl:attribute name="rest">
		    <xsl:for-each select="current-group()/tg/t/@rest">
